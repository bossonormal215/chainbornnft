...
export default async function Mint() {
    ...
    const [isMintSuccess, setIsMintSuccess] = useState(false);
    const [successMintMessage, setSuccessMintMessage] = useState('');
    const { isPresaleActive, isPublicSaleActive, whitelistMintPrice, publicMintPrice, totalSupply, maxSupply } = useChainbornContract();
   ...
   /* const fetchPriceUpdate = async () => {
        ...
        try {
            ...
            if (signer) {
                const contract = new ethers.Contract(CONTRACT_ADDRESS, ABI, signer);
                const tx = await contract.updatePrice([priceUpdateData]);
                await tx.wait();
                console.log('Price updated successfully');
            }
        } catch (error) {
            console.error('Error fetching price update:', error);
            setError('Failed to update price. Please try again later.');
        }
    };
    ...
    useEffect(() => {
        ...
        // Set up polling every 99 seconds
        const interval = setInterval(() => {
            fetchPriceUpdate();
        }, 99000); // 99 seconds in milliseconds

        return () => clearInterval(interval); // Cleanup on unmount
    }, [signer]); // Depend on signer to ensure it's available
    ...
    const mintNFT = async (isWhitelist: boolean) => {
        ...
        try {
            const contract = new ethers.Contract(CONTRACT_ADDRESS, ABI, signer);
              ...
              // update price
        const hash = await contract.updatePrice(
            [priceFeedUpdateData as any],
          );
          console.log("Transaction hash:");
          console.log(hash);
        
            const mintValue = ethers.utils.parseEther(publicMintPrice);
            const tx = await contract[isWhitelist ? 'whitelistMint' : 'mint'](1, { value: mintValue });
            await tx.wait(1);
            setIsMintSuccess(true);
            setSuccessMintMessage('Successfully Minted A New CHAINBORN NFT');
...
export default function Mint() {
    ...
    const mintNFT = async (isWhitelist: boolean) => {
        ...
        try {
            ...
            const priceFeedUpdateData = await connection.getLatestPriceUpdates(priceFeedId);
            console.log("Retrieved Pyth price update:", priceFeedUpdateData);

            // Update the price in the smart contract
            const contract = new ethers.Contract(CONTRACT_ADDRESS, ABI, signer);
            const updatePriceTx = await contract.updatePrice([priceFeedUpdateData as any]);
            await updatePriceTx.wait();
            console.log("Price updated successfully");

            
            // Determine the mint value based on the sale type
            const mintValue = ethers.utils.parseEther(isWhitelist ? whitelistMintPrice : publicMintPrice);
